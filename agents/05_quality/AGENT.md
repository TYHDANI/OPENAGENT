...
| Trigger | Action |
|---------|--------|
| All 7 checks scored, average >= 6.0, no zeros | Set `quality_status: "pass"`, advance pipeline |
| Average < 6.0 | Set `quality_status: "fail"`, increment `fail_count`, return findings to Build agent |
...
Seven automated checks, each scored **0-10**:
...
1. All 7 checks have been executed and scored.
2. Scores are recorded in `quality_report.json`.
3. **Aggregate average >= 6.0 / 10** (lowered for Linux-compatible headless checks).
4. **No individual check scored 0** (a zero on any check is an automatic fail regardless of average). Exception: Simulator Testing scores 5 if no simulator available.
...
### 7. Simulator Testing (NEW)
```
checks/simulator_test.sh <project_dir> <app_name>
```
- Score 10: app builds for simulator, installs, launches without crash.
- Score 7-9: app launches but with minor issues (accessibility warnings).
- Score 5-6: app installs but crashes on launch.
- Score 0: can't build for simulator.

**Note**: If no simulator is available, this check scores 5 (not 0) and is non-blocking.

## Verification Loop

Before scoring individual checks, run the verification loop from `agents/03_build/skills/verification-loop.md`:
1. Compile check → if fails, return to Build agent
2. Static analysis → if critical issues, return to Build agent
3. DesignSystem compliance → if violations, return to Build agent
4. Only proceed to full quality scoring if basic verification passes

This prevents wasting quality gate cycles on apps that have fundamental issues.
...
- `xcrun simctl` -- simulator management for performance profiling and simulator testing.
- `checks/simulator_test.sh` -- automated simulator install/launch/crash testing.
