#!/usr/bin/env bash
# OPENAGENT — 10_promo agent launcher (enhanced with video ads)
...
if [ ! -d "$PROJECT_DIR" ]; then
  echo "[10_promo] ERROR: Project directory not found: $PROJECT_DIR"
  exit 1
fi
...
# ── Generate video ad specs ──────────────────────────────────────
if [ -f "$ROOT_DIR/orchestrator/video_ads.sh" ]; then
  source "$ROOT_DIR/orchestrator/video_ads.sh"
  APP_NAME=$(python3 -c "import json; print(json.load(open('$STATE_FILE')).get('name','App'))" 2>/dev/null || basename "$PROJECT_DIR")
  CATEGORY=$(python3 -c "import json; print(json.load(open('$STATE_FILE')).get('category','productivity'))" 2>/dev/null || echo "productivity")
  generate_seedance_prompt "$PROJECT_DIR" "$APP_NAME" "$CATEGORY" 2>/dev/null || true
  generate_ugc_scripts "$PROJECT_DIR" "$APP_NAME" "saves time and money" "people" 2>/dev/null || true
  echo "[10_promo] Video ad specs generated."
fi

echo "[10_promo] Starting promo agent for: $(basename "$PROJECT_DIR")"
echo "[10_promo] Model: $MODEL ($(model_tier "$MODEL")) | Backend: $(get_backend "$MODEL")"

if run_with_model "$PROMPT" "$MODEL" "$PROJECT_DIR" "$STATE_FILE" "promo_materials_generated" "true"; then
  echo "[10_promo] Promo phase completed successfully."
  exit 0
else
  echo "[10_promo] ERROR: Promo agent failed."
  exit 1
fi
